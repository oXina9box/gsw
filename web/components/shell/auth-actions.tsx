...
export function AuthActions({ authenticated, userEmail }: { authenticated: boolean; userEmail?: string }) {
  ...
  if (!authenticated) return <><Link className="button button-outline" href="/login">Sign in</Link><Link className="button button-primary" href="/signup">Create Studio</Link></>;
