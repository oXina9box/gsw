...
export default function ResetPasswordPage() {
  ...
  return <section className="form-page" data-archetype="A2"><div className="form-card"><p className="kicker">Gem Studio / account</p><h1>Choose a new password.</h1><form onSubmit={submit}><label>New password<input type="password" minLength={8} required value={password} onChange={(event) => setPassword(event.target.value)} /></label><button className="button button-primary" type="submit">Update password</button></form>{error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-note" role="status">{message}</p>}</div></section>;
}
