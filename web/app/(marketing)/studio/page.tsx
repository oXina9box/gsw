...
export default function StudioPage() {
  return <article className="marketing-detail" data-archetype="A1">
    ...
    <section className="detail-band shell reveal-on-scroll"><div><h2>A brief becomes an operating plan.</h2></div><p>Choose the channel, audience, goal, rights confirmation, schedule, credit cap, run mode, and quality preference before a production moves. Manual, semi-automatic, and automatic runs share the same visible approval history.</p></section>
    <section className="department-rail shell reveal-on-scroll" aria-label="Studio departments">{DEPARTMENTS.map((department, index) => <div className="department-rail-item" key={department}><span>{String(index + 1).padStart(2, "0")}</span><strong>{department}</strong></div>)}</section>
    <section className="detail-grid shell reveal-on-scroll">
    ...
    <section className="detail-cta shell reveal-on-scroll"><h2>The floor is only useful when the handoffs hold.</h2><Link className="button button-outline" href="/system">Explore the system ↗</Link></section>
  </article>;
}
