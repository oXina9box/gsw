...
export default function SystemPage() {
  return <article className="marketing-detail" data-archetype="A1">
    ...
    <section className="process-list shell reveal-on-scroll">{steps.map(([number, title, copy]) => <article key={number}><span>{number}</span><div><h2>{title}</h2><p>{copy}</p></div></article>)}</section>
    <section className="run-mode-panel shell reveal-on-scroll"><div><h2>Choose where the system waits.</h2></div><div className="run-mode-list"><p><strong>Manual</strong><span>You start each next step.</span></p><p><strong>Semi-auto</strong><span>The workflow runs until a configured gate.</span></p><p><strong>Auto</strong><span>The workflow continues within policy, provider, and credit limits.</span></p></div></section>
    ...
    <section className="detail-cta shell reveal-on-scroll"><h2>A release is the start of the next signal.</h2><Link className="button button-outline" href="/social-workshop">Explore the social workshop ↗</Link></section>
  </article>;
}
