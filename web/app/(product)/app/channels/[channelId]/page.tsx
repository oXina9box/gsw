...
export default async function ChannelPage({ params, searchParams }: { params: Promise<{ channelId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  if (!channel) notFound();
  return <section className="product-page shell" data-archetype="B1-B"><Link className="text-link" href="/app/channels">← Channels</Link><div className="section-head"><div><span className="channel-tag">{channel.status}</span><h1>{channel.name}</h1></div><Link className="button button-primary" href="/app/front-office">Open production</Link></div>
