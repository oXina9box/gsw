...
Modal popup lifecycle: mandatory after auth until `onboarding_profiles.step === 'complete'`. Identity contract: `studioNameStatus` (`provided` | `deferred`), optional `studioName`, optional `logoAssetId` (max 5 MB SVG/PNG/WebP, server-validated), 1–3 `brandColors` (hex values with WCAG AA compliance), `contentDirectionStatus`, optional `contentDirection` taxonomy, optional `contentDescription`. Commercial contract: `cloud-1`, `cloud-2`, `cloud-3`, or `byok` (standalone or stacked). Provider contract: OpenAI/Anthropic connection with AES-256-GCM encrypted server-side storage, masked keys (`sk-...`), test verification. First Marketing lane workbench: Studio Brand and Channel Discovery handoffs with 6-file agent catalog contracts (`role`, `soul`, `jobdescription`, `skills`, `memory`, `user_content`) and mandatory user approval before persisting.
