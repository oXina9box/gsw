...
### Onboarding and first Marketing lane

`/app/onboarding` is an authenticated, resumable setup page. Incomplete setup never redirects, obscures, or disables other authenticated pages. Identity contract: `studioNameStatus` (`provided` | `deferred`), optional `studioName`, optional `logoAssetId` (max 5 MB SVG/PNG/WebP, server-validated), 1–3 `brandColors` (hex values with WCAG AA compliance), `contentDirectionStatus`, optional `contentDirection` taxonomy, optional `contentDescription`. Commercial contract: `cloud-1`, `cloud-2`, `cloud-3`, or `byok` (standalone or stacked). Provider contract: OpenAI/Anthropic connection with AES-256-GCM encrypted server-side storage, masked keys (`sk-...`), test verification. Channel, department, and first-lane sections are explicit writes; none runs from page load. First Marketing lane workbench keeps Studio Brand and Channel Discovery handoffs with 6-file agent catalog contracts (`role`, `soul`, `jobdescription`, `skills`, `memory`, `user_content`) and mandatory user approval before persisting.
